...
**Спринт:** 14C.3 — Directional Asset Import ✅ (directional PNG разложены; down/up/side у player и всех зомби; LEFT=flip; fallback цел)  
...
> Animation Pipeline (восстановление после отката): `systems/animation.py` (AnimState/AnimationComponent/AnimationClip/`draw_animated`) и его интеграция переустановлены. В `settings.py` возвращены `ANIMATION_FPS=8.0` и `PLAYER_ATTACK_ANIM_DURATION=0.2`. Сущности: `Player`/`Zombie`(Walker/Runner/Spitter)/`Boss` имеют поле `self.animation = AnimationComponent(self.SPRITE)`; в `update()` зовут `animation.play(state); animation.update(dt)`, в `draw()` — `draw_animated(surface, rect, animation, self.SPRITE)`. Выбор состояния: Player — attack(`_attack_anim_timer`, ставится в `fire()` при выстреле)>walk(движение)>idle; Zombie — маппинг `_ANIM_FROM_AISTATE` поверх AIState (PATROL/CHASE/REPOSITION→walk, ATTACK→attack, IDLE→idle); Boss — `_anim_state`: death(мёртв)>attack(в радиусе)>walk(видит)>idle, death выставляется в `take_damage` на переходе жив→мёртв. Сигнатуры update/draw/конструкторов не менялись; combat/EventBus/Quest/Save/AssetLoader/GameWorld не тронуты.
> Directional Animation (14C.2, Variant B): `Facing(DOWN/UP/LEFT/RIGHT)` + `facing_from_vector(dx,dy,current)` (доминирующая ось; нулевой вектор → текущее) в `systems/animation.py`. Арт нарисован для down/up/side (калибровка 14C.1): маппинг `_ART_TOKEN` DOWN→down, UP→up, RIGHT→side, LEFT→side. `AnimationComponent`: `set_facing()`/`facing`/`flip_horizontal`(=LEFT)/`directional_sprite`(`<prefix>_<state>_<art>_<i>`). `current_sprite` НЕ изменился (обратная совместимость старых тестов). Сущности вычисляют facing: Player — из ввода (последнее при стоянии), Zombie/Boss — из `player.pos - self.pos`; сигнатуры update/draw не менялись.
> Цепочка fallback рендера (14C.2): directional-кадр `<prefix>_<state>_<art>_<i>` (LEFT — тот же side, отражённый `pygame.transform.flip` ТОЛЬКО в рендере) → обычный кадр `<prefix>_<state>_<i>` → статический `self.SPRITE` → примитив. AssetLoader API не менялся (используются get()/draw_sprite). Тесты: `tests/test_directional_animation.py` (27).
> Directional Asset Import (14C.3): directional-PNG разложены в assets/sprites через `tools/import_sprites.py` (`_DIRECTIONAL` карта: game_prefix→state→token→исходные индексы; зомби-типы делят `_ZOMBIE_DIR`). Калибровка (контакт-листы 14C.1/14C.3): PLAYER walk down=18-21/up=30-33/side=112-115, idle down=52-53/up=64-65, attack down=0-1/up=12-13/side=7-8; ZOMBIE-риг (walker/runner/spitter/boss) walk down=52-55/side=58-61/up=64-67, attack down=1-2/side=3-4/up=12-13. token∈{down,up,side}; LEFT=side+flip (отдельного left-арта НЕТ). idle есть только у player; death — направленно-нейтральный (directional нет → база). Базовые кадры `<prefix>_<state>_<i>` и статика сохранены (717 PNG: 623 база/статика + 94 directional). Визуально подтверждено: down=лицо, up=спина, side=профиль. Тесты: `tests/test_directional_assets.py` (17). 1100 тестов зелёные, ruff/mypy зелёные. Игровой код (animation.py/AssetLoader/сущности) НЕ менялся — только импортёр + ассеты + тесты. Реальные PNG лежат в `assets/sprites/` (импортированы tools/import_sprites.py из набора `sprites/`; контракт `player_*`, `zombie_walker_*`, `zombie_runner_*`, `zombie_spitter_*`, `boss_*` по состояниям + статические `player.png`/`zombie_*`/`boss.png`/`npc_*`/`item_*`/`tile_*`). У зомби нет idle-кадров — idle уходит в статический fallback. Тесты «нет ассетов» переведены на изолированный пустой каталог (monkeypatch SPRITES_DIR), т.к. проект теперь отгружает реальные спрайты. QA: ruff/mypy зелёные, pytest 1056 зелёных.
